...
#running xgboost on train and valid sets
library(xgboost)
library(caret)
target <-  as.numeric(datatop100$target)

dim(datatop100)
xg_data <- datatop100
lapply(xg_data,as.numeric)
xg_validate <- dataForValidtop100
lapply(xg_validate,as.numeric)

dim(xg_data)
dim(xg_validate)

target_class <- xg_data$target
target_validate_class <- xg_validate$target_test
